const tarotDeck = [
  ["00", "The Fool", "คนพเนจร", "✦", "การเริ่มต้น", "เด็กหนุ่มถือโคมเดินเข้าเมืองที่ไม่มีแผนที่ เขาไม่รู้ว่าประตูบานใดปลอดภัย แต่เสียงระฆังบอกว่าโลกจะเปิดทางให้คนที่กล้าเริ่ม", "./assets/cards/samples/major-00-the-fool.png"],
  ...
  ["02", "The High Priestess", "นักบวชหญิงแห่งม่านหมอก", "☾", "สัญชาตญาณ", "ใต้หอสมุดต้องห้าม มีหญิงผู้เฝ้าประตูอ่านจดหมายที่ยังไม่ถูกส่ง เธอยิ้ม เพราะความจริงบางอย่างไม่ได้หายไป แค่รอให้ใจนิ่งพอจะได้ยิน", "./assets/cards/samples/major-02-high-priestess.png"],
  ...
  ["13", "Death", "ผู้เก็บเกี่ยวรุ่งสาง", "✝", "การสิ้นสุด", "เมื่อรถม้าของผู้เก็บเกี่ยวผ่านทุ่งเก่า ต้นไม้ไม่ได้ตายทั้งหมด บางกิ่งเพียงยอมทิ้งใบ เพื่อเก็บแรงไว้ผลิดอกฤดูหน้า", "./assets/cards/samples/major-13-death.png"],
...
].map(([number, english, thai, symbol, essence, story, image]) => ({ number, english, thai, symbol, essence, story, image }));
...
function renderReading(card, seeker, sequence) {
  ...
  const cardFace = node.querySelector(".card-face");
  article.style.animationDelay = `${Math.min(sequence - 1, 2) * 90}ms`;
  if (card.image) {
    cardFace.classList.add("has-art");
    cardFace.style.backgroundImage = `url("${card.image}")`;
  }
